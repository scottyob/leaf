...
bool tempRH_init() {  
  ...
  if (!tempRH_isConnected()) { 
    ...
    if (!tempRH_isCalibrated())  {
      ...
      while (tempRH_isBusy()) {
        delay(1);
        if (counter++ > 100)
          success = false; //Give up after 100ms
      }
  ...
  if(DEBUG_TEMPRH) {
    if (success) Serial.println("Temp_RH - SUCCESS: AHT20 Temp Humidity sensor calibrated!");
    else Serial.println("Temp_RH - FAIL: AHT20 Temp Humidity sensor not initialized/calibrated");
  }  
...
void tempRH_update(uint8_t process_step) {
  ...
  if (process_step == 1) {
    if(!currently_processing) tempRH_triggerMeasurement();  // if we haven't yet processed a prior measurement, don't trigger a new one
