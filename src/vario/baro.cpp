...
// Device Management
  ...
  {  
    ...
    if(DEBUG_BARO) {
      ...
      Serial.print("  TEMP:"); Serial.println(baro.temp);
      Serial.print("  OFF1:"); Serial.println(OFF1);
      Serial.print("  SENS1:"); Serial.println(SENS1);
      Serial.print("  P_ALT:"); Serial.println(baro.alt);
  ...
  char baro_update(bool startNewCycle, bool doTemp) {    // (we don't need to update temp as frequently, so we can skip it most of the time)
    ...
    switch (process_step) {
      ...
      case 2:     // READ TEMP THEN CALCULATE ALTITUDE
        ...
        // (even if we skipped some steps above because of mis-reads or mis-timing, we can still calculate a "new" corrected pressure value based on the old ADC values.  It will be a repeat value, but it keeps the filter buffer moving on time)
...
// Device reading & data processing  
  ...
  void baro_filterPressure(void) {  
    ...
    //old way with averaging last N values equally:
      ...
      pressureFilterVals[filterBookmark] = baro.pressure;          // load in the new value at the bookmarked spot
  ...
  void baro_updateClimb() {
    ...
    baro.climbRate = (baro.alt - lastAlt) * 20;	// climb is updated every 1/20 second, so climb rate is cm change per 1/20sec * 20
