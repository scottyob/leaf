...
void AltimeterMenuPage::draw() {
  ...
  do { 
    ...
    // First draw all the text for the labels and setting descriptions
      ...
      uint8_t menu_items_y[] = {190, 80, 115, 150}; // these are for selectable fields only (doesn't apply to standard altimeter value shown, which is not selectable)
    ...
    for (int i = 0; i <= cursor_max; i++) {      
      ...
      switch (i) {				
        ...
        case cursor_altimeter_syncToGPS:          
          if (ALT_SYNC_GPS) u8g2.print(char(125));
...
void AltimeterMenuPage::setting_change(Button button, ButtonState state, uint8_t count) {  
  ...
  int8_t invert = 1;    // flip direction of button actions depending if we're adjusting altimeter (height, + means go up in altitude) or altimeter setting (inHg, + means go up in pressure, which is DOWN in altitude)
  ...
  switch (cursor_position) {
    ...
    case cursor_altimeter_adjustSetting:
      if (button == Button::CENTER && count == 1 && state == HELD) {  // if center button held for 1 'action time'
        ...
        if (state == PRESSED || state == HELD || state == HELD_LONG) {
          if (button == Button::LEFT) baro_adjustAltSetting(-1*invert, count);
          else if (button == Button::RIGHT) baro_adjustAltSetting(1*invert, count);
